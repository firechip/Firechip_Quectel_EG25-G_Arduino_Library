/*
  ...
  This example demonstrates how to transfer data from one EG25-G to another using multiple TCP sockets.
  ...
  If you select "Ping":
    ...
    The code then opens multiple TCP sockets to the "Pong" EG25-G using port number TCP_PORT_BASE, TCP_PORT_BASE + 1, etc.
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  ...
  if (selection == 1) // Ping
  {
    ...
    for (int i = 0; i < numConnections; i++)
    {
      ...
      socketNum[i] = myEG25.socketOpen(EG25_G_TCP);
      if (socketNum[i] == -1)
      {
      ...
      if (myEG25.socketConnect(socketNum[i], theAddress, TCP_PORT_BASE + i) != EG25_G_SUCCESS)
      {
      ...
      const char ping[] = "Ping";
      myEG25.socketWrite(socketNum[i], ping); // Send the "Ping"
    }
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  ...
  else // if (selection == 2) // Pong
  {
    ...
    for (int i = 0; i < numConnections; i++)
    {
      ...
      if (myEG25.socketListen(socketNum[i], TCP_PORT_BASE + i) != EG25_G_SUCCESS)
      {
