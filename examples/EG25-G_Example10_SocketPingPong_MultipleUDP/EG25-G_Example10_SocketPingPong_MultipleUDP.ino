/*
  ...
  This example demonstrates how to ping-pong binary data from a EG25-G to UDP Echo Server using multiple UDP sockets.
  ...
  Here's a quick how-to (assuming you are familiar with Python):
    ...
    Copy the Multi_UDP_Echo.py from the GitHub repo Utils folder: https://github.com/firechip/Firechip_Quectel_EG25-G_Arduino_Library/tree/main/Utils
...
void processSocketData(int socket, const char *theData, int length, IPAddress remoteAddress, int remotePort)
{
  ...
  if ((theData[0] == 0x00) && (theData[1] == 0x01) && (theData[2] == 0x02) && (theData[3] == 0x03)) // Look for the "Ping"
  {
    if (pongCount[connection] < pingPongLimit)
    {
      ...
      myEG25.socketWriteUDP(socket, remoteAddress, remotePort, pong, 4); // Send the "Pong"
  ...
  if ((theData[0] == 0x04) && (theData[1] == 0x05) && (theData[2] == 0x06) && (theData[3] == 0x07)) // Look for the "Pong"
  {
    if (pingCount[connection] < pingPongLimit)
    {
      ...
      myEG25.socketWriteUDP(socket, remoteAddress, remotePort, ping, 4); // Send the "Ping"
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  ...
  myEG25.setSocketReadCallbackPlus(&processSocketData);
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  ...
  for (int i = 0; i < numConnections; i++)
  {
    ...
    socketNum[i] = myEG25.socketOpen(EG25_G_UDP);
    if (socketNum[i] == -1)
    {
    ...
    const char ping[] = { 0x00, 0x01, 0x02, 0x03 };
    myEG25.socketWriteUDP(socketNum[i], theAddress, UDP_PORT_BASE + i, ping, 4); // Send the "Ping"
  }
...
void loop()
{
  ...
  for (int i = 0; i < numConnections; i++)
  {
    if (((pingCount[i] >= pingPongLimit) || (millis() > (startTime + timeLimit))) && (socketNum[i] >= 0))
    {
      ...
      Serial.print(F("\r\nClosing socket "));
      Serial.println(socketNum[i]);
      myEG25.socketClose(socketNum[i]); // Close the socket
      socketNum[i] = -1;
    }
  }
} // /loop()
